label scene16:

    scene black with dissolve

    #music market
    play music storetheme
    "Eventually, we each go our separate ways, Misaki and Satan exit the supermarket early, to the sadness of the two demons and the indifference of myself."
    "Once we had everything we needed, we - {i}I{/i} paid and left, the voices of the twin demons squabbling behind me."

    scene store exteriornight with dissolve
    
    "We exit the superstore, the legion of people I saw before having vanished into the late afternoon." 
    "Where they go I don't know - probably to their unassuming, unexciting suburban life, if I had to guess."
    show lucy smile with easeinleft:
            align (0.30, 1.0)
    show yumi fneutral with easeinleft:
            align (0.0, 1.0)
    "I see my sister - the demon Lucy alongside her - at the car, and I push the trolley across the parking lot."
    "The weight feels almost Sisyphean, in a way."

    show yumi fneutralspeak
    voice "C-16-1.mp3" #Yumi (Kaito)
    sis "You done?"
    
    show yumi fneutral
    voice "C-16-2.mp3" #Makoto (Reece Bridger)
    pro "Yes. We have finished our… gathering."
    
    show yumi fsmiletalk
    voice "C-16-3.mp3" #Yumi (Kaito)
    sis "Cool."

    show yumi fsmile
    "She moves, hoisting up the trunk of the car above her head and letting it hang."

    show yumi fsmiletalk
    voice "C-16-4.mp3" #Yumi (Kaito)
    sis "Okay, put your stuff in--"
    
    #music abrupt cut
    stop music fadeout 0.5
    show yumi fsurpriseshout
    show lucy bigfrowntalk
    voice "C-16-5.mp3" #Mami 
    mm "{b}Uwaaaaa--!{/b}"

    "A scream - piercing, louder than the judgement of the gods - sounded through the parking lot. I turn, unsure of who the noise is coming from. A banshee? A mandrake? A child?"
    
    show mami panic with easeinright:
            align (0.85, 1.0)
    "...None of the above, apparently."

    #music intense
    play music intensetrack fadein 1.0
    voice "C-16-6.mp3" #Mami 
    mm "Where is it? {b}Where is it!?{/b}"
    
    
    show lucy neutraltalk
    show mami disgust
    voice "C-16-7.mp3" #Lucy (Vivi)
    lu "W-where's what?"

    "Personally, I'm wondering the same thing."

    show mami panic
    voice "C-16-8.mp3" #Mami 
    mm "M-My necklace... it's gone!"
    
    show yumi fworry
    show lucy bigfrowntalk
    voice "C-16-9.mp3" #Makoto (Reece Bridger)
    pro "...Gone?"
    
    show mami disgust
    voice "C-16-10.mp3" #Mami 
    mm "I've lost it, I've dropped it, I-I might not be able to have it again, I…"

    "She trails, looking off."
    "Honestly, I'm not sure how one could even lose something they had placed on their neck, but I suppose I wouldn't know. I've never worn one, after all."

    show bubble fsmiletalk with easeinright:
            align (1.05, 1.0)
    voice "C-16-11.mp3" #Bubble 
    bb "Aww, sucks that it {i}vanished,{/i} sis. How do you think you lost it?"
    
    show mami disgustclosed
    voice "C-16-12.mp3" #Mami 
    mm "I don't... I can't.. I... I had it."
    
    show mami sadclosedtalk
    show bubble fsurprise
    voice "C-16-13.mp3" #Mami 
    mm "Did I drop it…? No, of course not, but then..."

    "She seems... despondent, in a way. Incapable of response, as if she had seen what laid on the deepest circles of Hell and lived to tell the tale."
    "Of course, she almost certainly had."

    show lucy smile
    voice "C-16-14.mp3" #Lucy (Vivi)
    lu "Do you want to, um, find it?"
    
    show mami panic
    voice "C-16-15.mp3" #Mami 
    mm "I… please… I…"
    
    show bubble fsadsmiletalk
    #music cut
    stop music fadeout 1.0
    voice "C-16-16.mp3" #Bubble 
    bb "Don't we need to go homeeeeee, though?"
    
    show mami fscowl
    voice "C-16-17.mp3" #Mami 
    mm "...Are you stupid?"
    
    show bubble fsmileclosed
    voice "C-16-18.mp3" #Bubble 
    bb "...No?"
    
    show lucy bigfrowntalk
    show yumi fsurpriseshout
    show mami fshout
    voice "C-16-19.mp3" #Mami 
    mm "{i}Do you not understand the importance of this necklace?{/i}"
    
    show bubble fsmiletalk
    voice "C-16-20.mp3" #Bubble 
    bb "...Why should I?"
    
    show lucy angst
    show yumi fneutral
    show mami scowlclosed
    voice "C-16-21.mp3" #Mami 
    mm "Tch. Typical."

    show bubble fsmileclosed
    show yumi fexasperatedtalk
    #music yumi's theme
    play music calmtrack fadein 1.0
    "My sister sighs."

    voice "C-16-22.mp3" #Yumi (Kaito)
    sis "...We can wait a bit, I suppose. I can pack the car while you go look."
    
    show lucy serioustalk
    show yumi exasperated
    show mami scowl
    voice "C-16-23.mp3" #Lucy (Vivi)
    lu "...And I can help! I've got an eye for detail, so, um…"
    
    show mami sadtalk
    voice "C-16-24.mp3" #Mami 
    mm "Y-You can help?"
    
    show mami sad
    voice "C-16-25.mp3" #Makoto (Reece Bridger)
    pro "Me too, if this bothers you that much."

    "Truth be told, I would rather be heading home by now - but given Mami's… reaction, I suppose that there wasn't much of a choice. As the summoner, I seemed to be her master, and what sort of master would voluntarily let their subjects suffer?"
    "A lot of masters, probably, but not me. I give the trolley to my sister, nod as she takes it, and walk to where the other three are."
    hide yumi with easeoutleft
    "Contented with the arrangement, Yumi moves to unload the cart." 
    show mami neutral with easeinright:
            align (0.6, 1.0)
    "The other three look at me expectantly. Their eyes bore into me. The words that come out of my mouth are stilted. Stuttery."


    voice "C-16-26.mp3" #Makoto (Reece Bridger)
    pro "So, uh, what do we do?"
    
    show lucy smile
    voice "C-16-27.mp3" #Lucy (Vivi)
    lu "We look. I know how to look for these things, so, uh…"
    
    show lucy neutral
    show mami scowl
    show bubble fxdshout
    voice "C-16-28.mp3" #Bubble 
    bb "This is {i}boooooorrrrrrrriiiiinnnnnnggggg{/i}..."
    
    voice "C-16-29.mp3" #Makoto (Reece Bridger)
    pro "Go help my, uh, sister then."
    
    
    show bubble smileclosed
    #sfx downer
    voice "C-16-30.mp3" #Bubble 
    bb "Okkkkaaaaayyyyyyyyyyyy..."

    #music cut
    stop music fadeout 1.0
    hide bubble with easeoutright
    "She leaves, though in a different direction. I get the feeling she has no plans to help Yumi..."
    "Well, now it's up to us to look through the labyrinthine parking lot."
    hide mami with easeoutleft
    hide lucy with easeoutright
    "We take our separate paths wordlessly for the sake of efficiency."
    
    scene parking with dissolve
    #music calm
    play music calmtrack fadein 1.0
    "Now that the sun is nearly gone from the sky, there are almost no mortals inhabiting this parking lot. That is… convenient. Preferable, even. If I had to bump into someone, if I had to talk to someone who had no idea what the depths of this world were, I..."
    "...Wouldn't know what to do. And {i}God{/i} knew what would happen if the Demon Lord Sebastian Wolfgang IV was caught off guard."
    "But now was not the time for unlikely hypotheticals. I had a necklace I needed to find. I turned a corner and saw a new part of the parking lot. Cars. Abandoned trolleys. Litter."
    "Eventually, I see a glint under a conspicuously-placed rock out on the grass. I walk forward, slightly curious. Is that what I'm looking for? Is that the thing Mami covets so dearly? I'm not entirely sure."
    "Still, I suppose there is no harm in looking. I walk to the dumpster, pick the… chain thing up, and--"

    show mami shout with easeinright:
        align (0.6, 1.0)
    voice "C-16-31.mp3" #Mami 
    mm "{b}Mine!{/b}"

    "A force appears with the speed of a lightning bolt and wrenches the thing from my hands just as I am about to look at it. My body jolts. I look around. The sensation, the {i}thoughts{/i} are familiar. Is this…?"

    show mami sadsmiletalk
    voice "C-16-32.mp3" #Mami 
    mm "You found it. {i}You found it~!{/i}"

    show mami smugclosed
    "In front of me is Mami, bent over slightly. She is clutching, caressing the necklace, She isn't looking at me, nor the two demons coming up from behind her; she is only looking at the object in her hands."
    "Her {i}precious{/i} possession, as expected from the lord of greed."
    "I question how much it means, given her… extreme reaction."

    show mami neutral
    voice "C-16-33.mp3" #Makoto (Reece Bridger)
    pro "What's so important about that necklace, anyway? You're fawning over it."

    show mami sad
    "To be honest, I would have preferred to ask why losing it prompted such a reaction from her, but I suppose that this is the… better way of asking it."

    show mami sadtalk
    voice "C-16-34.mp3" #Mami 
    mm "It's…"
    stop music fadeout 1.0

    show mami scowl
    "She pauses. Shakes her head. Continues."

    show mami sadsmiletalk
    #music sad
    play music sadtheme fadein 1.0
    voice "C-16-35.mp3" #Mami 
    mm "It's a necklace that my friend Leviathan gave to me, back… around a year ago, I guess."
    
    show mami smugcomment
    voice "C-16-36.mp3" #Mami 
    mm "Admittedly it… might not be the most valuable; admittedly, I only took it because Levi made it, but…"
    
    show mami smug
    voice "C-16-37.mp3" #Mami 
    mm "That's sort of the reason, y'know?"
    
    show mami sadclosedtalk
    voice "C-16-38.mp3" #Mami 
    mm "Like, Levi's a friend. She's not really my {i}bestest{/i} friend or anything like that but, like…"
    
    show mami lecture
    voice "C-16-39.mp3" #Mami 
    mm "I still value her."
    
    show mami sadsmile
    voice "C-16-40.mp3" #Mami 
    mm "I still care about her."
    
    show mami sadclosedtalk
    voice "C-16-41.mp3" #Mami 
    mm "And I care enough to know that she's the type of person who doesn't really make friends easily, who compares herself to others and looks down on everything bad they do."
    
    show mami sadtalk
    voice "C-16-42.mp3" #Mami 
    mm "But I know that she sees herself as worse than others as well. I know she's jealous of other people. I know she wants to be on their level. I know that she expects what she makes to be the best, I know she wants others to care about her things."
    
    show mami stoic
    voice "C-16-43.mp3" #Mami 
    mm "So how would she feel if I had suddenly lost the necklace she had made for me?"
    
    show mami scowl
    voice "C-16-44.mp3" #Mami 
    mm "How would she feel if it seemed like what she did hadn't mattered?"

    "Honestly, I don't know anything about this 'Levi' person, but I nod. The least I can do is pretend I understand."

    show mami sadsmiletalk
    voice "C-16-45.mp3" #Mami 
    mm "And it's… like that with everyone. All my friends. All my family. I try to keep the things they give me. I try to show them that their gifts matter."
    
    show mami sadtalk
    voice "C-16-46.mp3" #Mami 
    mm "I don't want to lose them; I don't want them to dislike me... I want to show them that they're important; I want to show them that I care."
    
    show mami sadsmile
    voice "C-16-47.mp3" #Mami 
    mm "I guess that's why this necklace is important to me."

    "Hm. Interesting."
    "I knew that Mami in front of me was the embodiment of greed..."
    "...but I supposed I never really saw that possessiveness as a good thing."
    "Hoarding things, keeping them just because you value the people who gave them to you."
    "I guess I will need to hold Mami in a different light."

    #music cut
    stop music fadeout 1.0
    show bubble bigsmileclosedtalk with easeinleft:
            align (0.4, 1.0)
    show lucy bigfrowntalk with easeinleft:
            align (0.15, 1.0)
    voice "C-16-48.mp3" #Bubble 
    bb "Aww, you found it? I thought I did a good job of hiding it…"

    show mami angrytalk
    "Bubble and Lucy walk onto the scene. What was Bubble doing here?"
    "And Lucy looked like she was on on the end of the nerves. Of course, then there was Mami..."

    show mami shout
    voice "C-16-49.mp3" #Mami 
    mm "{i}You{/i} hid this?"
    
    voice "C-16-50.mp3" #Bubble 
    show bubble smileclosed
    bb "Yep!"
    
    show lucy angst
    show mami shoutvein
    voice "C-16-51.mp3" #Mami 
    mm "You…"
    
    voice "C-16-52.mp3" #Bubble 
    show bubble smiletalk
    show mami shout
    bb "...got like, super flustered and stuff."

    #music lighthearted
    play music lightheartedtheme fadein 1.0
    
    show mami lecture
    "She giggles. You hear about twins finishing each other's sentences, but… I'm pretty sure that isn't what Mami's thinking."
    "What Mami's actually thinking...  it would {i}probably{/i} raise the age rating of this game."

    show bubble bigsmileclosedtalk
    show mami stammer
    voice "C-16-53.mp3" #Bubble 
    bb "It was great! You were like 'wa, wa, my precious necklace was gone,' and--"

    show mami scowl
    "Mami gives a sort of scoffing sound."

    show lucy neutral
    show mami angrytalk
    voice "C-16-54.mp3" #Mami 
    mm "Of course someone like {i}you{/i} wouldn't understand."
    
    show bubble sadsmiletalk
    voice "C-16-55.mp3" #Bubble 
    bb "Huh?"
    
    show mami shout
    voice "C-16-56.mp3" #Mami 
    mm "You just guzzle and consume whatever you can get your hands on and fail to take any of the {i}value{/i} out of it!"
    
    show bubble smileclosed
    voice "C-16-57.mp3" #Bubble 
    bb "...Yeah. So?"

    show mami disgustclosed
    "A moment of silence, before Mami sighs."

    show mami scowlclosedtalk
    voice "C-16-58.mp3" #Mami 
    mm "Whatever, no point in arguing. Let's just go home. {i}One{/i} of us needs to act like an adult, after all."
    
    show bubble bigsmiletalk
    show mami scowlclosed
    voice "C-16-59.mp3" #Bubble 
    bb "Oh, right. There's something I need to tell you."

    show bubble smile
    "She turns to me."

    show bubble pitytalk
    voice "C-16-60.mp3" #Bubble 
    bb "Your sis got tired of waiting so she decided to take the stuff home without us."

    #music cut
    stop music fadeout 1.0
    show lucy bigfrowntalk
    show mami panic
    "..."
    show mami shout 
    "{i}What.{/i}"
    
    voice "C-16-61.mp3" #Mami 
    mm "{i}What.{/i}"
    
    voice "C-16-62.mp3" #Lucy (Vivi)
    lu "Wh-What?"

    show mami scowl
    show bubble happytalk
    voice "C-16-63.mp3" #Bubble 
    bb "Yeah, she said that we could just walk home. Apparently doesn't take that long."

    show bubble smile
    "Of all the--"
    

    #sfx sweatdrop
    voice "C-16-64.mp3" #Mami 
    show lucy neutral
    show mami sadclosedyell
    mm "Fine. Let's just… walk, yeah."
    
    show lucy smile
    voice "C-16-65.mp3" #Lucy (Vivi)
    lu "W-We might be able to make it back before sundown if w-we begin now."
    
    show mami lecture
    voice "C-16-66.mp3" #Mami 
    mm "Right. I guess we start walking now?"
    
    voice "C-16-67.mp3" #Makoto (Reece Bridger)
    pro "Yes. We should."

    scene black with dissolve
    "And off we went."

    jump scene17
